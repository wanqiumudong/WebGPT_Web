...
class ColPaliManager:
    ...
    def load_model(self):
        ...
        try:
            ...
            _adapter_model_path = "/data/yphu/Web-FabGPT/LLM/RagManager/models/colpali/colpali-v1.3"
    ...
    def setup_milvus(self, collection_name):
        ...
        try:
            ...
            client_params = {
                "uri": "http://localhost:19530",  # Milvus Standalone的默认地址
            }
            
            # 创建数据库名称 (每个知识库使用单独的数据库)
            # 对collection_name进行sanitize处理，替换Milvus不支持的字符
            # 注意：collection_name应该已经在user_manager中进行过sanitize处理
            # 这里再次确保Milvus兼容性（连字符转下划线，空格转下划线）
            sanitized_collection_name = collection_name.replace('-', '_').replace(' ', '_')
            db_name = f"rag_{sanitized_collection_name}"
    ...
    def process_file(self, file_path, output_dir, doc_id=None, task_id=None, progress_callback=None, config=None):
        ...
        try:
            ...
            try:
                ...
                if existing_records:
                    logger.info(f"文档 {doc_id_int} 在Milvus中找到 {len(existing_records)} 条记录，检查页面文件是否存在")
                    
                    # 检查页面文件是否真实存在
                    doc_output_dir = os.path.join(output_dir, str(doc_id_int))
                    pages_exist = os.path.exists(doc_output_dir) and len(os.listdir(doc_output_dir)) > 0 if os.path.exists(doc_output_dir) else False
                    
                    if pages_exist:
                        logger.info(f"文档 {doc_id_int} 的页面文件存在，跳过处理")
                        # 创建一个简单的文档信息对象
                        ...
                        logger.warning(f"文档 {doc_id_int} 在Milvus中有记录但页面文件缺失，将重新处理")
                        # 清理Milvus中的无效记录
                        try:
                            delete_expr = f"doc_id == {doc_id_int}"
                            self.retriever.client.delete(
                                collection_name=self.retriever.collection_name,
                                filter=delete_expr
                            )
                            logger.info(f"已清理文档 {doc_id_int} 的无效Milvus记录")
                        except Exception as delete_err:
                            logger.error(f"清理无效Milvus记录失败: {str(delete_err)}")
                        # 继续执行正常的处理流程
