...
from .root_path import neural_ilt_root_path

import sys
# sys.path.append(neural_ilt_root_path)
...
from .dataloader.refine_data_loader_one_image import ILTRefineDataset

from .utils import unet_torch as unet_torch
...
class Neural_ILT_Wrapper:
    ...
    def neural_ilt_correction_for_one_image(self,dataloaders,out_image_root_path,target_path,**kwargs):
        import os
        output_mask_path=os.path.join(os.path.dirname(out_image_root_path),os.path.basename(target_path).split(".")[0]+"_neural_mask.png")
        ...
        for iteration in range(self.refine_iter_num): # Maximum on-neural-network ILT correction iterations
            ...
            with torch.set_grad_enabled(True):
                ...
                if iteration % 2 == 0:
                    print(
                        "time: %.2fs\titer: [%d/%d]\tobj_loss: %.2f\tl2_loss: %.2f\tcplx_loss: %.2f\tepe_vio: %d"
                        % (
        ...
        best_image_path = output_mask_path
        if self.save_mask:
            ...
            #     neural_ilt_root_path,"output/refine_net_output", "%s_res.png" % (layout_name)
            # )
            best_image_path = os.path.join(
                out_image_root_path,os.path.basename(target_path).split(".")[0]+"_mask.png"
            )
            print("Saving best mask in %s" % best_image_path)
            torchvision.utils.save_image(mask_origin, best_image_path)
        
        output_mask_path= best_image_path 
        output_image_path=None
        if kwargs.get("output_result",True):
        # 这里得看看printedNom要不要乘255
            printed=self.refine_backbone_model.get_printed(inputs, new_cord)
            output_image_path=os.path.join(out_image_root_path,os.path.basename(target_path).split(".")[0]+"_nerual_printed.png")
            # import pdb;pdb.set_trace()
            printed_crop = torch.nn.functional.interpolate(
                printed, size=(abs(y2 - y1), abs(x2 - x1)), mode="nearest"
            )  # 1 * 1 * H * W
            printed_origin = torch.zeros(
                (printed_crop.shape[0],printed.shape[0], 2048, 2048),
                dtype=printed_crop.dtype,
                layout=printed_crop.layout,
                device=printed_crop.device,
            )
            printed_origin[..., y1:y2, x1:x2]=printed_crop
            torchvision.utils.save_image(printed_origin, output_image_path)
            # neural-ilt没法指定
        ...
        if kwargs.get("output_metrics",True):
            ...
            output_message="ImageName: %s\tTime: %.2fs\tbest_iter: %.2f\tbest_loss-> total:%.2f,\tl2:%.2f,\tcplx:%.2f,\tEPEV:%d"%(
                layout_name,
                cur_runtime,
                best_iter,
                best_loss,
...
def nerual_ilt_opt_wrapper(target_path,output_image_root_path, **kwargs):
    exp_para = {
        ...
        "ilt_model_path": os.path.join(neural_ilt_root_path,"models/unet/", args.load_model_name),
    ...
    lithosim_para = {
        "kernels_root": neural_ilt_root_path+"lithosim/lithosim_kernels/torch_tensor",
        "kernel_num": 24,
    }
    ...
    output= nerual_ilt.neural_ilt_correction_for_one_image(refine_data_loader,output_image_root_path,target_path,**kwargs)
    
        
    # output_image_path=out_mask_path
    return output



def nerual_ilt_sim_wrapper(mask_path,output_image_root_path, **kwargs):
    exp_para = {
        "device": "cuda:%s" % args.gpu_no if torch.cuda.is_available() else "cpu",
        "phase": "test",
        "beta": args.beta, # hyper-parameter for cplx_loss
