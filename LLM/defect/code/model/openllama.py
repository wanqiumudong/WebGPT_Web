...
import kornia as K
from .feature_visualize import show_feature_map

import torch
from torch.nn.utils import rnn
# import easyocr

CLASS_NAMES = ['SEM-image', 'bottle', 'cable', 'capsule', 'carpet', 'grid', 'hazelnut', 'leather', 'metal nut', 'pill',
               'screw', 'tile', 'toothbrush', 'transistor', 'wood', 'zipper', 'object',
               'candle', 'cashew', 'chewinggum', 'fryum', 'macaroni', 'pcb', 'pipe fryum', 
               'Hole', 'Particle', 'Pattern_Deform', 'Scratch', 'sem']
...
#                         ]
objs = ['SEM-image', 'bottle', 'cable', 'capsule', 'carpet', 'grid', 'hazelnut', 'leather', 'metal nut', 'pill', 'screw',
        ...
        'pcb2', 'pcb3', 'pcb4', 'capsules', 'Hole', 'Particle', 'Pattern_Deform', 'Scratch', 'sem']
...
class OpenLLAMAPEFTModel(nn.Module):
    ...
    def __init__(self, **args):
        ...
        self.args = args
        self.model_parallel = str(args.get('model_parallel', '0')).lower() in ['1', 'true', 'yes', 'on']
        self.llm_device_map = args.get('llm_device_map', 'auto')
        main_device = args.get('main_device', 'cuda:0')
        self.device = torch.device(main_device if torch.cuda.is_available() else 'cpu')
        ...
        llama_load_kwargs = {}
        use_mp = self.model_parallel and torch.cuda.is_available() and torch.cuda.device_count() > 1
        if use_mp:
            # 通过HF自动切分将LLaMA分配到多张卡，降低单卡显存压力
            llama_load_kwargs = {
                'device_map': self.llm_device_map,
                'torch_dtype': torch.float16,
                'low_cpu_mem_usage': True
            }
            print(f'[ModelParallel] Enable llama device_map={self.llm_device_map}, cuda_count={torch.cuda.device_count()}')
        self.llama_model = LlamaForCausalLM.from_pretrained(vicuna_ckpt_path, **llama_load_kwargs)
        ...
        self.prompts_sim = nn.Parameter(torch.randn((1, 4096), device=self.device), requires_grad=True)
    ...
    def prompt_wraped(self, img_embeds, input_ids, target_ids, attention_mask, anomaly_embedding=None):
        '''
        ...
        if anomaly_embedding != None:
            # img_embeds_broadcasted = img_embeds.expand_as(p_after_embeds)
            # cosine_similarities = F.cosine_similarity(img_embeds_broadcasted, p_after_embeds, dim=2)
            # softmax_scores = (cosine_similarities + 1) / 2
            # weighted_img_embeds = torch.mean(softmax_scores.unsqueeze(2) * img_embeds, dim=1)
            # img_embeds = weighted_img_embeds.unsqueeze(1)
            #
            # anomaly_embedding_reshaped = anomaly_embedding.unsqueeze(2)
            # p_after_embeds_reshaped = p_after_embeds.unsqueeze(1)
            # anomaly_embedding_cosine_similarities = F.cosine_similarity(anomaly_embedding_reshaped, p_after_embeds_reshaped, dim=3)
            # anomaly_embedding_softmax_scores = (anomaly_embedding_cosine_similarities + 1)/2
            # anomaly_embedding = torch.mean(anomaly_embedding_softmax_scores.unsqueeze(3) * anomaly_embedding_reshaped, dim=2)

            B, s2, embed = p_after_embeds.shape
            promptsim = self.prompts_sim.expand(B, -1, -1)
            promptsim = promptsim.expand_as(p_after_embeds)
            cosine_similarities = F.cosine_similarity(promptsim, p_after_embeds, dim=2)
            # 将结果归一化到0-1之间
            normalized_similarity = 0.5 + 0.5 * cosine_similarities
            weighted_img_embeds = torch.mean(normalized_similarity.unsqueeze(2) * img_embeds, dim=1)
            img_embeds = weighted_img_embeds.unsqueeze(1)
    ...
    def prompt_wrap(self, ocr_text, img_embeds, input_ids, target_ids, attention_mask, anomaly_embedding=None):
        '''
            input_ids, target_ids, attention_mask: bsz x s2
        '''
        input_ids = input_ids.to(self.device)  # bsz x s2
        ...
        p_ocr_tokens = self.llama_tokenizer(ocr_text, return_tensors="pt", add_special_tokens=False).to(self.device)
        p_ocr_embeds = self.llama_model.model.model.embed_tokens(p_ocr_tokens.input_ids).expand(batch_size, -1,
                                                                                                      -1)
        ...
        if anomaly_embedding != None:
            ...
            inputs_embeds = torch.cat(
                [bos_embeds, p_before_embeds, img_embeds, p_middle_embeds, anomaly_embedding, p_after_embeds, p_ocr_embeds],
                dim=1)  # bsz x (1+s1+1+s2) x embed_dim
            # create targets
            empty_targets = (
                torch.ones([batch_size,
                            1 + p_before_embeds.size()[1] + 1 + p_middle_embeds.size()[1] + anomaly_embedding.size()[
                                1] + p_ocr_embeds.size()[1]],  # 1 (bos) + s1 + 1 (image vector)
                           dtype=torch.long).to(self.device).fill_(-100)
            )  # bsz x (1 + s1 + 1)
            targets = torch.cat([empty_targets, target_ids], dim=1)  # bsz x (1 + s1 + 1 + s2)
            assert inputs_embeds.size()[1] == targets.size()[1]
            ...
            atts_prefix = torch.ones([batch_size, 1 + p_before_embeds.size()[1] + 1 + p_middle_embeds.size()[1] +
                                      anomaly_embedding.size()[1] + p_ocr_embeds.size()[1]], dtype=torch.long).to(
                self.device)  # bsz x (1 + s1 +1)
            attention_mask = torch.cat([atts_prefix, attention_mask], dim=1)
            assert attention_mask.size() == targets.size()  # bsz x (1 + s1 + 1 + s2)
            return inputs_embeds, targets, attention_mask
        else:
            inputs_embeds = torch.cat([bos_embeds, p_before_embeds, img_embeds, p_middle_embeds, p_after_embeds, p_ocr_embeds],
                                      dim=1)  # bsz x (1+s1+1+s2) x embed_dim
            # create targets
            empty_targets = (
                torch.ones([batch_size, 1 + p_before_embeds.size()[1] + 1 + p_middle_embeds.size()[1] + p_ocr_embeds.size()[1]],
                           # 1 (bos) + s1 + 1 (image vector)
                           dtype=torch.long).to(self.device).fill_(-100)
            )  # bsz x (1 + s1 + 1)
            targets = torch.cat([empty_targets, target_ids], dim=1)  # bsz x (1 + s1 + 1 + s2)
            ...
            atts_prefix = torch.ones([batch_size, 1 + p_before_embeds.size()[1] + 1 + p_middle_embeds.size()[1] + p_ocr_embeds.size()[1]],
                                     dtype=torch.long).to(self.device)  # bsz x (1 + s1 +1)
            attention_mask = torch.cat([atts_prefix, attention_mask], dim=1)
            assert attention_mask.size() == targets.size()  # bsz x (1 + s1 + 1 + s2)
            return inputs_embeds, targets, attention_mask
    ...
    def forward(self, inputs):
        ...
        if 'masks' in inputs:
            ...
            input_ids, target_ids, attention_mask = process_batch_instance(self.llama_tokenizer, output_texts,
                                                                           self.max_tgt_len)
            # inputs_embeds, targets, attention_mask = self.prompt_wrap(ocr_texts, img_embeds, input_ids, target_ids, attention_mask,
            #                                                           anomaly_map_prompts)

            inputs_embeds, targets, attention_mask = self.prompt_wraped(img_embeds, input_ids, target_ids,
                                                                      attention_mask,
                                                                      anomaly_map_prompts)
            ...
            input_ids, target_ids, attention_mask = process_batch_instance(self.llama_tokenizer, output_texts,
                                                                           self.max_tgt_len)
            inputs_embeds, targets, attention_mask = self.prompt_wraped(img_embeds, input_ids, target_ids, attention_mask,
                                                                      anomaly_map_prompts)
    ...
    def extract_multimodal_feature_new(self, file_path):
        features = []
        if file_path:
            c_name = 'object'

            image_embeds, _, patch_tokens = self.encode_image_for_web_demo(file_path)
    ...
    def extract_multimodal_feature(self, inputs, web_demo):
        ...
        if inputs['image_paths']:
            ...
            # src = inputs['image_paths'][0]
            # parts = src.rsplit('.', 1)[0].split('/')    # 这会选取'bottle'到'014'之间的所有部分
            # # 将提取的部分拼接起来，并去除'.png'
            # desired_part = '/'.join(parts[-4:])
            # # 现在替换'/'为'_'
            # result = desired_part.replace('/', '_')
            # if 'good' not in result:
            #     show_feature_map(src, anomaly_map_ret, result)
    ...
    def prepare_generation_embedding(self, inputs, web_demo):
        ...
        # 将结果归一化到0-1之间
        normalized_similarity = 0.5 + 0.5 * cosine_similarities
        weighted_img_embeds = torch.mean(normalized_similarity.unsqueeze(2) * feature_embeds, dim=1)
        feature_embeds = weighted_img_embeds.unsqueeze(1)
