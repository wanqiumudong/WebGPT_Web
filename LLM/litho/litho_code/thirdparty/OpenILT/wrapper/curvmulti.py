...
def curvmulti_opt(target_path,out_image_root_path,**kwargs):
    import os
    output_mask_path=os.path.join(out_image_root_path,os.path.basename(target_path).split(".")[0]+"_curvmulti_mask.png")
    ScaleLow = 8
    ScaleMid = 4
    ScaleHigh = 2


    cfgLow = CurvILTCfg(openilt_root_path+"./config/curvilt256.txt")
    cfgMid = CurvILTCfg(openilt_root_path+"./config/curvilt512.txt")
    cfgHigh = CurvILTCfg(openilt_root_path+"./config/curvilt1024.txt")
    ...
    # High resolution
    design = glp.Design(target_path, down=ScaleHigh)
    ...
    if kwargs.get("output_result",True):
        ...
        # Print Information
        print(f"Result: Low: {logLow} -> Mid: {logMid} -> High: {logHigh}; Runtime: {runtime:.2f}s")
    mask, printedNom, printedMax, printedMin = test.sim(bestMask, target, scale=ScaleHigh)
    ...
    if kwargs.get("output_result",True):
        ...
        output_image_path=os.path.join(out_image_root_path,os.path.basename(target_path).split(".")[0]+"_curvmulti_printed.png")
    ...
    if kwargs.get("output_metrics",True):
        output_message=f"Result: Low: {logLow} -> Mid: {logMid} -> High: {logHigh}; Runtime: {runtime:.2f}s"
