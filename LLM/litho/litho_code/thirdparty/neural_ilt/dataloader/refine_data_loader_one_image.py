...
# neural_ilt_root_path="/data/Web-FabGPT/LLM/litho/litho_code/thirdparty/neural-ilt/"


# def data_process(image_path,
#         margin=128,
#         scale_dim_w=512,
#         scale_dim_h=512,
#         split="ibm_opc_test",
#         read_ref=False,):
#     # 默认read_ref=False,不做处理
...
#                 size=(scale_dim_w, scale_dim_h), resample=Image.NEAREST
#             )
#             image_tensor = transforms(resized_img)
...
class ILTRefineDataset(data.Dataset):
    ...
    def __init__(
        ...
        if os.path.isfile(self.data_root):
            self.layouts = [self.data_root]
            
        else:
            raise RuntimeError("folder not supported")
    ...
    def load_refine_dataset(self, data_root, split):
        ...
        with open(split_list_path, "r") as split_list:
            for line in split_list.readlines():
                ...
                layouts.append(os.path.join(neural_ilt_root_path, image_path))
        return layouts
