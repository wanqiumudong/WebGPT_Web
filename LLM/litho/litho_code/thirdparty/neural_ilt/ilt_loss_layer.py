...
from .utils import ilt_utils as ilt
...
class ilt_loss_function(Function):
    ...
    @staticmethod
    def get_printed(mask_pred, target, kernels, kernels_ct, weight):
...
class ilt_loss_layer(nn.Module):
    ...
    def get_printed(self, preds, target,new_cord=None):
        if new_cord is None:
            return ilt_loss_function.get_printed(preds, target, self.kernels, self.kernels_ct, self.weight)
        else:
            ...
            return ilt_loss_scale_function.get_printed(preds, target, self.kernels, self.kernels_ct, self.kernel_def, self.kernel_def_ct, self.weight, self.weight_def,new_cord, self.cycle_mode, self.cplx_obj, self.report_epe)
